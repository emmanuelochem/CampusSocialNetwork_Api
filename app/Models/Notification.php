...
class Notification extends DatabaseNotification
{
    ...
    const CRUSHED = 5;
    const SENT_MESSAGE = 6;
    const REPLIED_CHAT = 7;
    const MENTIONED = 8;
...
//    }
    ...
    public function getMessageAttribute()
    {
        ...
        if ($data['action'] === self::CRUSHED) {
            return "has a crush on you.";
        }

        if ($data['action'] === self::SENT_MESSAGE) {
            return "sent you a message.";
        }

        if ($data['action'] === self::REPLIED_CHAT) {
            return "replied your chat.";
        }

        if ($data['action'] === self::MENTIONED) {
